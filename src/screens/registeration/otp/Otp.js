...
const Otp = (props) =>
{
  ...
  useEffect(() =>
  {
    (async function ()
    {
      const natsUser = await AsyncStorage.getItem("natsUserSeed");
      ...
      if (!natsUser)
      {
        ...
        await AsyncStorage.multiSet([['natsUserSeed', new TextDecoder().decode(user.getSeed())], ['publicNats', user.getPublicKey()]]);
      }
    })();
  }, []);
